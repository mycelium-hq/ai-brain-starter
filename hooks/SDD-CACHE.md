...
> Cherry-picked verbatim from [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) (MIT) 2026-05-26.
...
```
   ...
   `${CLAUDE_PROJECT_DIR}` resolves to the directory you launched Claude Code from. The snippet above works when the hooks live inside the same project. If you installed `ai-brain-starter` elsewhere (e.g. as a shared plugin under `~/agent-skills`), replace `${CLAUDE_PROJECT_DIR}/hooks/...` with the absolute path to each script.
...
Cherry-picked verbatim from `addyosmani/agent-skills` under MIT License.
