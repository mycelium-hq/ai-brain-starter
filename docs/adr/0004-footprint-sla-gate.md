...
- **Per-event / per-tool substrate cold-start fan-out** vs `footprint-budgets.json`. This is the CPU/battery + reliability axis. **[Corrected by MYC-2359 / [ADR 0005](0005-cache-position-injected-context.md): this ADR originally *excluded* `once: true` entries here "because they fire once per session." They do NOT — `once` is ignored in settings.json (the installer's merge target), so a `once` UPS hook re-fires every message. The gate now COUNTS all substrate-python entries and flags any `once: true` as a hard dead-flag breach (axis E).]** `PreToolUse` / `PostToolUse` are matcher-gated, so fan-out is computed per tool (`Write`, `Bash`, …). Only commands that resolve to a shipped `skills/ai-brain-starter/(hooks|scripts)/X.py` are counted — `[ -f ~/.claude/hooks/… ]`-guarded maintainer hooks no-op on a fresh install and are excluded.
