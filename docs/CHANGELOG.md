...
**Who this affects:** no one yet. Both bugs were introduced by the same in-progress change that added the merge carve-out to `guard-session-artifacts-on-default-branch.sh` (the pre-commit / pre-merge-commit hook that stops session-close artifacts from stranding on a topic branch instead of landing on the default branch), and both were caught and fixed in review before any of it reached the default branch.
