...
## 2026-09-25: the off-main session-artifact guard could be silently defeated — a failed git call, or a fresh orphan branch

**Who this affects:** no one yet. Both bugs were introduced by the same in-progress pull request against `guard-session-artifacts-on-default-branch.sh` (the pre-commit / pre-merge-commit hook that stops session-close artifacts from stranding on a topic branch instead of landing on the default branch) — by a later commit in it, the one that batched the exemption lookup into two `git` calls, not by the commit that added the merge carve-out — and both were caught and fixed in review before any of it reached the default branch.

**Bug 1 — a failing `git diff-index` read as "nothing changed".** Both exemption routes (origin/`<default>`, MERGE_HEAD) fed their `git diff-index` call through `done < <(...)`, which only ever exposes the trailing `read`'s own exit status, never the command feeding it. If that `diff-index` call failed outright — too many arguments on a huge pathspec, an unreadable object, an index lock — it produced empty output, and empty output was read as "zero candidates differ": every staged artifact on that route was exempted, including ones that had never existed on the ref at all. Each route's output now goes to a real temp file with its exit status checked directly; a failed comparison is now treated exactly like a missing ref — it just doesn't run, it never grants an exemption.

**Bug 2 — an unborn branch listed no staged paths at all.** The guard's very first step lists everything staged with `git diff --cached --name-only -z --no-renames HEAD`. On a fresh `git checkout --orphan` branch, before its first commit, `HEAD` does not resolve to anything yet — the explicit `HEAD` argument made that call fail and produce no output, which read as "nothing is staged." The guard exited ALLOW before the artifact-matching loop ever ran, no matter what was actually staged. Dropping the explicit `HEAD` lets `git diff --cached` fall back to its own default: compare against HEAD when it exists, against the empty tree when it doesn't. This listing's own failure — for any reason, not just an unborn branch — now refuses instead of silently allowing, since every later check depends on it.

**Round 3 (the same unmerged pull request) — three more, from an independent review of the fix above.** The scratch directory the exemption check writes to was still allocated by the older, less reliable route, so a single allocation failure switched the whole merge carve-out off: a commit whose content already matched the default branch was refused instead of exempted. Both scratch directories now come from one shared routine, so they cannot disagree. A refusal that happened *because* no scratch directory could be made used to report itself as "the comparison failed", naming causes that had not occurred and sending you to re-run something that would never change; that state now says what actually happened. And the hook now deletes its own leftover `guard-staged.*` directories inside `.git` when they are more than a day old, since nothing else reaps that location — a live run's directory, a like-named file, and any other prefix are all left alone, and three of that check's four assertions are negative controls for exactly that.
