...
**Who this affects:** anyone relying on `guard-session-artifacts-on-default-branch.sh` (the pre-commit / pre-merge-commit hook that stops session-close artifacts from stranding on a topic branch instead of landing on the default branch).
